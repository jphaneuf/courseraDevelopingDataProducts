...
shinyServer(
  function(input,output){
    output$newHist <- renderPlot({
      ...
      par(mfrow=c(2,1))
      plot(time,y,type="l",col="red",ylim=c(-2,2))
      titleStr <- paste0("sine wave, amplitude of 1 at ",as.character(sf),"Hz")
      title(titleStr)
      ...
      #plot((1:(n/2))*1e6/4096,abs(fft(y))[1:(n/2)]/(n/2),type="l",lwd=3)
    })
  }
)